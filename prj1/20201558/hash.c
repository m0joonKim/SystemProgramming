...
struct hash* createHash(){
  ...
  hash_init(newHash, hash_int_fun, lessHash,NULL);
  return newHash;
}
struct hash_elem* makeHashElem(int data){
  struct hash_elem* newHashElem = (struct hash_elem*)calloc(1,sizeof(struct hash_elem));
  newHashElem->data = data;
  return newHashElem;
}
void printHashElem(struct hash_elem* e, void* aux){
  printf("%d ",e->data);
}

void squareHashElem(struct hash_elem* e, void* aux){
  e->data = e->data*e->data;
}
void tripleHashElem(struct hash_elem* e, void* aux){
  e->data = e->data*e->data*e->data;
}
void removeHashElem(struct hash_elem* e, void* aux){
  free(e);
  return ;
}

void printHash(struct hash* h){
  if(hash_size(h)==0)return ;
  hash_apply(h,printHashElem);
  printf("\n");
}
void applyHash(struct hash* h, char* com){
  if(!strcmp(com,"square")){
    hash_apply(h,squareHashElem);
  }else if(!strcmp(com,"triple")){
    hash_apply(h,tripleHashElem);
  }
}
unsigned hash_int_fun(const struct hash_elem *e, void *aux){
  //얘로 hash_int써서 넘겨줘야함.
