...
void printBitmap(struct bitmap** BITMAP, char* bitmapName){
  int idx = atoi(bitmapName+2);
  if(BITMAP[idx]==NULL){
    return ;
  }
  struct bitmap* bm = BITMAP[idx];
  size_t i, j, len = bm->bit_cnt;
  for (i = 0; i < len; ++i) {
  //  printf("%ld", (bm->bits[i / ELEM_BITS] >> (i % ELEM_BITS)) & 1);
    if(bitmap_test(bm,i)){
      printf("1");
    }else{
      printf("0");
...
struct bitmap* bitmap_expand(struct bitmap* bm, size_t exSize){
  struct bitmap* newBitmap = bitmap_create(bm->bit_cnt+exSize);
  for(size_t i=0;i<bm->bit_cnt;i++){
    if((bm->bits[i / ELEM_BITS] >> (i % ELEM_BITS)) & 1==true){
      bitmap_set(newBitmap,i,true);
    }else{
      bitmap_set(newBitmap,i,false);
    }
  }
  free(bm);
  return newBitmap;
